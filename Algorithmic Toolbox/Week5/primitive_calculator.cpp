...
std::vector<int> dp_sequence(int n, std::vector<std::pair<int, int>> &MinOps) {
    ...
    for (int i = 2; i <= n; i++) {
        ...
        if (MinOps[div3] <= MinOps[div2] && MinOps[div3] <= MinOps[add1] && MinOps[div3] <= MinOps[i]) {
            MinOps[i] = std::make_pair(MinOps[div3].first + 1, div3);

        }
            // if div2 is smallest
        else if (MinOps[div2] <= MinOps[div3] && MinOps[div2] <= MinOps[add1] && MinOps[div2] <= MinOps[i]) {
            MinOps[i] = std::make_pair(MinOps[div2].first + 1, div2);
        }
            // if add1 is smallest
        else {
            MinOps[i] = std::make_pair(MinOps[add1].first + 1, add1);
        }
    }
...
int main() {
    int n;
    //std::cin >> n;
    std::vector<std::pair<int, int>> MinOps((unsigned int) 5 + 1, std::make_pair(INT_MAX, INT_MAX));
    std::vector<int> sequence = dp_sequence(5, MinOps);
